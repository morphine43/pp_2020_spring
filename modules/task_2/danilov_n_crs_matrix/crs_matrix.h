...
#ifndef MODULES_TASK_2_DANILOV_N_CRS_MATRIX_CRS_MATRIX_H_
#define MODULES_TASK_2_DANILOV_N_CRS_MATRIX_CRS_MATRIX_H_
...
class CRSMatrix {
 ...
 public:
    ...
    friend inline CRSMatrix<T> ompMultiply(const CRSMatrix<T>& left, const CRSMatrix<T>& right) {
        if (left.columnsNumber != right.rowsNumber)
            throw "Error: the number of columns in the left matrix does not equal "
                  "the number of rows in the right matrix.";
...
#endif  // MODULES_TASK_2_DANILOV_N_CRS_MATRIX_CRS_MATRIX_H_
