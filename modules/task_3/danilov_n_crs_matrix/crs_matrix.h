...
#ifndef MODULES_TASK_3_DANILOV_N_CRS_MATRIX_CRS_MATRIX_H_
#define MODULES_TASK_3_DANILOV_N_CRS_MATRIX_CRS_MATRIX_H_
...
class CRSMatrix {
 ...
 public:
    ...
    friend inline CRSMatrix<T> tbbMultiply(const CRSMatrix<T>& left, const CRSMatrix<T>& right) {
        if (left.columnsNumber != right.rowsNumber)
            throw "Error: the number of columns in the left matrix does not equal "
                  "the number of rows in the right matrix.";
        ...
        tbb::parallel_for(tbb::blocked_range<std::size_t>(0, result.getrowsNumber()),
                          CRSMatrixMultiplicator<T>(left, right, result));
...
class CRSMatrixMultiplicator {
 private:
    const CRSMatrix<T>& left;
    const CRSMatrix<T>& right;
    CRSMatrix<T>& result;

 public:
    inline CRSMatrixMultiplicator(const CRSMatrix<T>& a, const CRSMatrix<T>& b,
                                  CRSMatrix<T>& result) : left(a), right(b), result(result) {
    }

    void operator()(const tbb::blocked_range<std::size_t>& r) const {
        T sum = T{0};
        for (std::size_t i = r.begin(); i < r.end(); ++i) {
            for (std::size_t j = 0; j < right.getcolumnsNumber(); ++j) {
                for (std::size_t k = 0; k < left.getcolumnsNumber(); ++k)
                    sum += left.get(i, k) * right.get(k, j);
...
#endif  // MODULES_TASK_3_DANILOV_N_CRS_MATRIX_CRS_MATRIX_H_
